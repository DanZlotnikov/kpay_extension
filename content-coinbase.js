        ...
        if (addressRequest.response == "null" || addressRequest.response == null) {
			...
			if (response.EthAddresses == null || response.EthAddresses == {} ||  response.EthAddresses == false){
				...
				if (currency == 'eth') {
					...
					address = getLeastUsedAddress(response.BtcAddresses);
				}
			}
        }
        address_box.select();
...
function getDomainAddress(domain) {
    ...
    addressRequest.open("GET", url, true);
    addressRequest.send();
}
...
usageRequest.onreadystatechange = function () {
    if (usageRequest.readyState == 4 && usageRequest.status == 200) {
...
DocBody = document.evaluate('/html/body', document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
DocBody.onmouseover = getAddressBox;


function getAddressBox() {
    xpath = '//div[@class="ReactModalPortal"]/div/div/div[2]/div[2]/div[2]/div/div[2]/div/div/div/input';
    address_box = document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
    if (address_box) {
        address_box.onclick = popupKPay;
    }
}

function popupKPay() {
    currencyHeaderXpath = '//div[@class="ReactModalPortal"]/div/div/div[1]/h4';
    currencyHeader = document.evaluate(currencyHeaderXpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
    if (currencyHeader.innerHTML.includes("BTC")) {
        currency = 'btc';
    }
    else if (currencyHeader.innerHTML.includes("ETH")) {
        currency = 'eth';
    }
